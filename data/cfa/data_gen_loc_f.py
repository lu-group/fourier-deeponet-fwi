...
if __name__ == "__main__":
    ...
    for ii in [2, 3, 4]:
        for iii in range(36):
            print(ii, iii)
            f_list = np.load(f"./f/f{ii}_1_{iii}.npy").astype(np.float32).reshape(num_samples, 1)
            loc_list = np.load(f"./loc/loc{ii}_1_{iii}.npy").astype(np.float32).reshape(num_samples, 5)
            vp = np.load(f"./velocity/vel{ii}_1_{iii}.npy").astype(np.float32)
            ...
            np.save(f'./seismic/loc_f/seis{ii}_1_{iii}.npy',seis)
